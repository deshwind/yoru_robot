...
<style>
  ...
  * { box-sizing:border-box; margin:0; min-width:0;
  ...
  .evbar { display:flex; align-items:center; gap:12px; flex-wrap:wrap;
           margin-bottom:14px; }
  .ev-kpis { display:flex; gap:12px; flex-wrap:wrap; margin-bottom:16px; }
  .ev-kpi { padding:13px 18px; min-width:118px; border-radius:18px; }
  .ev-kpi b { display:block; font-size:23px; font-weight:700; letter-spacing:-.02em; }
  .ev-kpi span { font-size:11px; color:var(--dim); text-transform:uppercase;
                 letter-spacing:.05em; }
  .ev-grid { display:grid; gap:14px;
             grid-template-columns:repeat(auto-fit,minmax(min(300px,100%),1fr)); }
  .ev-grid .panel { padding:16px; }
  .ev-grid canvas, #ev-cFsm { max-height:260px; }
  .ev-gal { display:grid; gap:10px;
            grid-template-columns:repeat(auto-fill,minmax(140px,1fr)); }
  .ev-gal figure { margin:0; }
  .ev-gal img { width:100%; border-radius:12px; border:1px solid var(--hairline);
                cursor:zoom-in; background:rgba(0,0,0,.2); aspect-ratio:4/3;
                object-fit:cover; }
  .ev-gal figcaption { font-size:10px; color:var(--dim); margin-top:3px;
                       word-break:break-all; }
  .ev-empty { color:var(--dim); font-size:13px; }
  #ev-lb { position:fixed; inset:0; background:rgba(0,0,0,.86); display:none;
           align-items:center; justify-content:center; z-index:60; padding:20px; }
  #ev-lb img { max-width:94%; max-height:94%; border-radius:12px; }
...
<div id="app" class="hidden">
  <aside class="glass">
    ...
    <button class="navbtn" data-view="evidence" onclick="go('evidence')">
      <span class="ic">&#128202;</span> Evidence</button>
  ...
  <main>
    ...
    <div id="view-evidence" class="view">
      <div class="evbar">
        <button onclick="loadEvidence()">&#8635;&nbsp; Refresh</button>
        <span class="note" id="ev-meta" style="margin:0">&mdash;</span>
      </div>
      <div class="ev-kpis" id="ev-kpis"></div>
      <div class="ev-grid">
        <div class="panel glass"><h2>Detections per class</h2>
          <canvas id="ev-cClass"></canvas></div>
        <div class="panel glass"><h2>Detection confidence</h2>
          <canvas id="ev-cConf"></canvas></div>
        <div class="panel glass"><h2>Incident outcomes</h2>
          <canvas id="ev-cOut"></canvas></div>
        <div class="panel glass"><h2>Incidents per room</h2>
          <canvas id="ev-cRoom"></canvas></div>
      </div>
      ...
      <div class="panel glass" style="margin-top:14px">
        <h2>Escalation frames (simulation)</h2>
        <div class="ev-gal" id="ev-galS"></div></div>
      <p class="note">Generated by the <code>evidence/</code> toolkit on the
        laptop (annotate_detections.py, capture_sim_evidence.py). Numbers are
        measured from real runs &mdash; no accuracy metrics are fabricated.</p>
    </div>
  </main>

  <div id="ev-lb" onclick="this.style.display='none'"><img id="ev-lbi" alt=""></div>
  ...
  <nav id="tabbar" class="glass">
    ...
    <button data-view="history" onclick="go('history')">
      <span class="ic">&#128203;</span>History</button>
    <button data-view="evidence" onclick="go('evidence')">
      <span class="ic">&#128202;</span>Evidence</button>
  </nav>
</div>
...
/* ------------- evidence analytics ------------- */
const EV = { ac:'#0a84ff', gr:'#34c759', pu:'#7b2dd6', rd:'#ff453a',
             or:'#ff9f0a', gy:'#8a93a6' };
let evCharts = {};

async function evImg(img, url) {
  // <img> cannot send the auth header: fetch with token and use a blob URL
  try {
    const r = await fetch(url, { headers: { 'X-Auth': token } });
    if (!r.ok) return;
...
function evLightbox(src) {
  document.getElementById('ev-lbi').src = src;
  document.getElementById('ev-lb').style.display = 'flex';
}

function evGallery(elId, label, names) {
  const el = document.getElementById(elId);
  if (!names || !names.length) {
    el.innerHTML = '<span class="ev-empty">Nothing yet &mdash; run the '
                 + 'evidence toolkit on the laptop.</span>';
    return;
  }
  el.innerHTML = '';
  names.forEach(n => {
    ...
    img.loading = 'lazy';
    img.onclick = () => evLightbox(img.src);
    const cap = document.createElement('figcaption');
    cap.textContent = n;
    fig.appendChild(img); fig.appendChild(cap); el.appendChild(fig);
    evImg(img, '/api/evidence/img/' + label + '/' + encodeURIComponent(n));
  });
}

function evChart(id, type, labels, data, color, opts) {
  if (!window.Chart) return;
  if (evCharts[id]) evCharts[id].destroy();
  const ctx = document.getElementById(id);
  evCharts[id] = new Chart(ctx, {
    type,
    data: { labels, datasets: [{ data,
      backgroundColor: Array.isArray(color) ? color : color,
      borderColor: EV.ac, pointBackgroundColor: EV.rd }] },
...
async function loadEvidence() {
  let d;
  try { d = await api('/api/evidence/data'); }
  catch (e) { return; }
  const ds = d.detection_summary || {};
  document.getElementById('ev-meta').textContent =
    `${d.n_incidents} incidents · ${ds.total_detections || 0} detections · `
    + `${ds.mean_fps ? ds.mean_fps + ' FPS' : 'n/a'} · model ${ds.primary_model || '-'}`;

  const kpis = [['Incidents', d.n_incidents], ['Detections', ds.total_detections || 0],
                ['Images', ds.images || 0], ['Mean FPS', ds.mean_fps || '-'],
                ['Latency ms', ds.mean_latency_ms || '-']];
  document.getElementById('ev-kpis').innerHTML = kpis.map(k =>
    `<div class="ev-kpi glass"><b>${k[1]}</b><span>${k[0]}</span></div>`).join('');

  const keys = o => Object.keys(o || {});
  const vals = o => keys(o).map(k => o[k]);
  const palette = [EV.gr, EV.rd, EV.or, EV.pu, EV.ac, EV.gy];
  evChart('ev-cClass', 'bar', keys(d.class_counts), vals(d.class_counts), EV.gr);
  const confLabels = d.conf_bins.map((_, i) => (i / 10).toFixed(1));
  evChart('ev-cConf', 'bar', confLabels, d.conf_bins, EV.pu);
  evChart('ev-cOut', 'doughnut', keys(d.outcomes), vals(d.outcomes), palette);
  evChart('ev-cRoom', 'bar', keys(d.rooms), vals(d.rooms), EV.ac);

  // FSM timeline -> stepped line over time
  const tl = d.fsm_timeline || [];
  if (window.Chart) {
    if (evCharts['ev-cFsm']) evCharts['ev-cFsm'].destroy();
    if (tl.length) {
      const order = [];
      tl.forEach(p => { if (!order.includes(p[1])) order.push(p[1]); });
      const pts = tl.map(p => ({ x: p[0], y: order.indexOf(p[1]) }));
      evCharts['ev-cFsm'] = new Chart(document.getElementById('ev-cFsm'), {
        type: 'line',
        data: { datasets: [{ data: pts, stepped: true, borderColor: EV.ac,
                             pointBackgroundColor: EV.rd, pointRadius: 4 }] },
        options: { plugins: { legend: { display: false } }, scales: {
          x: { type: 'linear', title: { display: true, text: 'time (s)' } },
          y: { min: 0, max: order.length - 0.5,
               ticks: { stepSize: 1, callback: v => order[v] || '' } } } },
      });
    }
  }
